...
def my_policy(obs):
    ...
    for i in range(num_agents):
        actions[i, 0] = 0  # Steering (always go straight)
        # actions[i, 0] = np.random.uniform(-0.1, 0.1)
        actions[i, 1] = 1  # Throttle (always accelerate)
        actions[i, 2] = 0  # Braking (no braking)
    return actions
